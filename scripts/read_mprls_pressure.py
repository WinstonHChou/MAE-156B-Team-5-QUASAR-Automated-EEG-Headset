        ...
        try:
            while True:
                ...
                if raw.startswith(KEY):
                    ...
                    mr = re.search(fr"Pressure_rate_kPa_s_{sensor_idx}:([-+]?\d*\.?\d+)", raw)
                    ...
                    pressure_rate_kpa_s = float(mr.group(1)) if mr else None

                    # log to CSV (include weight if present)
                    if weight_g is None or pressure_rate_kpa_s is None:
