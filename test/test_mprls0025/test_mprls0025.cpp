#include <unity.h>
#include "config.h"
#include "tca9548a.h"
#include "mprls0025pa00001a.hpp"

#define TEST_DURATION_MS 1000
#define MPRLS_ADDR MPRLS_DEFAULT_ADDR

// Sensor object
mprls0025pa00001a mpr = mprls0025pa00001a();

// Unit tests
void test_mprls0025_pressure(void) {
  Serial.begin(115200);
  long start_time = millis();
  TEST_MESSAGE("Starting MPRLS 0025 pressure sensor test...");
  for (uint8_t i = TCA9548A_MIN_CHANNEL; i <= TCA9548A_MAX_CHANNEL; i++) {
    int res = tcaselect(i);
    TEST_ASSERT_EQUAL(0, res); // expect success (0) when TCA present/responding

    Wire.beginTransmission(MPRLS_ADDR);
    if (!Wire.endTransmission()) {
      TEST_MESSAGE("TCA Port #");
      char buf[4];
      snprintf(buf, sizeof(buf), "%u", i);
      TEST_MESSAGE(buf);
      TEST_MESSAGE("Found MPRLS at address 0x");
      char addr_buf[4];
      snprintf(addr_buf, sizeof(addr_buf), "%02X", MPRLS_ADDR);
      TEST_MESSAGE(addr_buf);

      bool begun = mpr.begin(MPRLS_ADDR);
      TEST_ASSERT_TRUE(begun);

      uint8_t buffer[4]; // buffer to hold raw data from sensor
      while (millis() - start_time < TEST_DURATION_MS) {
        mpr.requestData();
        delay(5);
        const uint32_t& raw_val = mpr.readData(buffer);
        float pressure = mpr.convertToPressure(raw_val);
        TEST_MESSAGE("Pressure reading (kPa): ");
        char pres_buf[16];
        snprintf(pres_buf, sizeof(pres_buf), "%.2f", pressure);
        TEST_MESSAGE(pres_buf);

        delay(MPRLS_SAMPLING_INTERVAL_MS);
      }
    }
  }
  TEST_MESSAGE("Done");
}

static void run_all_tests(void) {
  UNITY_BEGIN();
  RUN_TEST(test_mprls0025_pressure);
  UNITY_END();
}

void setup() {
  Wire.begin(SDA_PIN, SCL_PIN);
  run_all_tests();
}

void loop() {
  // nothing
}
